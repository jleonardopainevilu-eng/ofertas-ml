let TODOS = [];
    ...
    function verTodasOrigen(origen) {
    ...
    function aplicarComunaDetectada(comunaTexto) {
      const select = document.getElementById("filtro-comuna");
      if (!select || !comunaTexto) return "";
      const detectada = normalizar(comunaTexto);
      let match = "";
      for (const opt of select.options) {
        if (!opt.value) continue;
        if (detectada.includes(opt.value) || opt.value.includes(detectada)) {
          match = opt.value;
          break;
        }
      }
    ...
    function buscarCercaDeMi() {
      const btn = document.getElementById("btn-cerca-mi");
      if (!navigator.geolocation) {
        alert("Tu navegador no permite compartir tu ubicación. Elige tu comuna manualmente en el filtro.");
        return;
      }
      ...
      navigator.geolocation.getCurrentPosition(async (posicion) => {
        try {
          ...
          const resp = await fetch(`https://api.bigdatacloud.net/data/reverse-geocode-client?latitude=${latitude}&longitude=${longitude}&localityLanguage=es`);
          const datos = await resp.json();
          const comuna = repararTexto(datos.locality || datos.city || datos.principalSubdivision || "");
          const match = aplicarComunaDetectada(comuna);

          if (!match) {
            alert(comuna
              ? `Detectamos "${comuna}", pero por ahora no tenemos ofertas filtradas para esa comuna. Te mostramos todas.`
              : "No pudimos identificar tu comuna. Elígela manualmente en el filtro.");
          }

          document.getElementById("ofertas")?.scrollIntoView({ behavior: "smooth", block: "start" });
        } catch (e) {
          alert("No pudimos detectar tu comuna automáticamente. Elígela manualmente en el filtro.");
        } finally {
          if (btn) {
            btn.disabled = false;
        ...
        alert("No pudimos acceder a tu ubicación. Revisa los permisos de ubicación del navegador o elige tu comuna manualmente.");
        if (btn) {
          btn.disabled = false;
          btn.textContent = textoOriginal;
        }
    ...
    function enviarWhatsAppPedido(producto, comuna, mascota, detalle) {
      ...
      window.open(`https://wa.me/56936493037?text=${msg}`, "_blank", "noopener");
    }
    ...
    function whatsappLink(numero, mensaje) {
      ...
      if (!limpio) return "sumar-tienda.html";
    ...
    function renderGrid() {
      ...
      const flyersMiembros = [
        renderFlyerCard("/assets/haz_crecer_tu_negocio_local.webp", "Haz crecer tu negocio local", 1),
        renderFlyerCard("/assets/tu_tienda_para_amantes_de_mascotas.webp", "Tu tienda podría estar acá", 2),
        renderFlyerCard("/assets/destaca_tus_ofertas_del_mes.webp", "Destaca tus ofertas del mes", 3)
      ];
      ...
      localGrid.innerHTML = locales.length
        ...
        : `<div class="vacio"><div class="vacio-icon">🐾</div><h3>Pronto verás tiendas locales acá.</h3><p>¿Tienes un negocio para mascotas? Súmate y muestra tus productos a nuevos clientes.</p><a class="btn btn-primary empty-cta" href="sumar-tienda.html">Sumar mi tienda</a></div>`;
    ...
    function renderFlyerCard(src, alt, idx = 0) {
      return `
        <a class="flyer-card" href="sumar-tienda.html" aria-label="${escaparHTML(alt)}" onclick="registrarClic('Flyer membresía: ${escaparHTML(alt)}')" style="animation-delay:${Math.min(idx * 0.04, 0.6)}s">
    ...
    function renderPromoCard(titulo, descripcion, textoBoton, emoji, idx = 0) {
      const linkContacto = "sumar-tienda.html";
      return `
